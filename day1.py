...
#             digitsInI.append(character)
#     strippedList.append(digitsInI)
#     digitsInI = []


# summy = 0
# for i in strippedList:
#     digits = int(str(i[0]) + str(i[-1]))
#     summy += digits
...
def check_substring(sub_string, is_rare):
    ...
    check_list = [('1','one'), ('2','two'), ('3','three'), ('4','four'), ('5','five'), ('6','six'), ('7','seven'), ('8','eight'), ('9','nine')]
...
def get_value(string_name):
    ...
    for element in string_name:
        if(element.isdigit()):
            first = check_substring(string_name[:string_name.index(element)], False)
            if(first == -1):
                first = element
            break
    ...
    for element in reversed(string_name):
        if(element.isdigit()):
            second = check_substring(string_name[string_name.rindex(element)+1:],True)
            if(second == -1):
                second = element
            break
